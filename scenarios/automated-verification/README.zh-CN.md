...
适合: 业务规则、解析、权限检查、UI component、bug 回归。

不适合: mock 太多，最后测试已经不像真实行为。

常见工具和做法: Jest、Vitest、pytest、JUnit、XCTest、React Testing Library、Flutter widget tests。

### Web UI E2E

适合: signup、checkout、invite、settings、admin action 等关键浏览器流程。

不适合: 用 E2E 测每个视觉细节。E2E 更适合流程，不适合所有组件状态。

常见工具和做法: Playwright、Cypress、Selenium、Storybook interaction tests。

### 移动端 E2E

适合: 原生 app 流程、onboarding、支付、权限和设备相关行为。

不适合: widget 或 integration test 能抓住风险时，一上来就做全设备测试。

常见工具和做法: Maestro、Detox、Appium、XCUITest、Espresso。

### API 和 contract 回归

适合: 后端服务、公开 API、webhook、auth 行为和集成。

不适合: 集合依赖脆弱共享状态或手动 secrets。

常见工具和做法: Bruno、Postman、Pact、Schemathesis、OpenAPI validators、supertest。

### AI 应用 eval

适合: RAG、agents、分类、抽取、审核和生成回复。

不适合: 用一个 golden prompt 判断整个 AI 功能。需要 dataset 和失败样本 review。

常见工具和做法: Braintrust、Langfuse、OpenAI Evals 思路、promptfoo、自定义 evaluation harness。

### CI 质量门禁

适合: 应该阻止 merge 或 release 的检查。

不适合: 慢且 flaky 的 gate。它会训练大家忽略失败。
