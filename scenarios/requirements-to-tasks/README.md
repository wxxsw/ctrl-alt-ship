...
Recommended when: small product changes, UI fixes, one endpoint, or a bug with known expected behavior.

Avoid when: cross-team work, unclear product decisions, or changes with compliance, billing, or migration risk.

Common tools and practices: GitHub Issues, Linear, Jira, plain Markdown, issue templates.

### PRD or RFC to task split

Recommended when: the product direction exists, but engineers need scoped slices and sequencing.

Avoid when: using a PRD as a substitute for unresolved product decisions.

Common tools and practices: Notion, Confluence, Google Docs, Linear projects, Jira epics, GitHub milestones.

### Acceptance example or BDD style

Recommended when: behavior depends on roles, states, edge cases, or user-visible messages.

Avoid when: turning every tiny task into ceremony. A few examples are often enough.

Common tools and practices: Gherkin-style examples, Given/When/Then notes, test case tables, Storybook stories for UI states.

### AI-assisted task shaping

Recommended when: you already know the intent and want help finding gaps, splitting work, or drafting checks.

Avoid when: letting the assistant decide scope, pricing, permissions, or rollout policy without a human decision.

Common tools and practices: chat assistants, IDE assistants, coding agents, internal planning templates.
