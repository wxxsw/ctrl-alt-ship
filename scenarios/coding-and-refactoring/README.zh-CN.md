...
推荐在: 需要多文件编辑，并且能通过命令行验证的任务。

避免在: 未经批准就运行影响生产、写外部系统或处理 secrets 的命令。

常见工具和做法: coding agent、本地 sandbox、branch workflow、CI feedback。

### Codemod 或编译器支持的迁移

推荐在: rename、API 迁移、框架升级和重复模式改造。

避免在: 语义变化太多，语法转换无法表达真实意图。
