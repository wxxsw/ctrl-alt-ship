...
适合: 行为、范围、架构、可读性、产品匹配和风险判断。

不适合: 先纠结风格，却漏掉任务漂移和缺证据。

常见工具和做法: GitHub/GitLab PR review、review template、CODEOWNERS、pair review。

### 静态和语义检查

适合: 可重复的代码质量、类型、格式、安全和依赖问题。

不适合: 加工具之前没决定哪些失败应该阻止 merge。
...
不适合: 慢且 flaky 的 pipeline，把真实信号藏起来。

常见工具和做法: GitHub Actions、GitLab CI、Buildkite、CircleCI、required status checks。

### AI 辅助 review

适合: 总结大 diff、指出风险区域、检查 PR 是否匹配任务卡。

不适合: 把 AI review 当作批准。它只是 reviewer signal，不是 ownership。
