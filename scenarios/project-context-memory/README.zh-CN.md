...
推荐在: 启动命令、测试命令、项目结构、代码约定和 review 预期。
...
推荐在: 大 repo、monorepo、陌生老系统或新人 onboarding。

避免在: 逐个列文件。地图应该说明决策在哪里发生。

常见工具和做法: 目录说明、Mermaid 图、架构概览页、依赖图。

### 决策记忆

推荐在: 以后还会被问到的架构、安全、数据和产品取舍。

避免在: 把历史写得过于漂亮，掩盖当时真实的取舍。

常见工具和做法: ADR、RFC archive、decision log、docs-as-code。

### Assistant 上下文层

推荐在: 团队反复在同一个 repo 里使用 coding agent 或 IDE assistant。

避免在: 提交个人 prompt、私密凭证或敏感本地 agent 配置。
